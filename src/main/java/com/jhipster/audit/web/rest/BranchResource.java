...
/**
 * REST controller for managing {@link com.jhipster.audit.domain.Branch}.
 */
@RestController
@RequestMapping("/api")
public class BranchResource {
    ...
    public ResponseEntity<Branch> updateBranch(@PathVariable(value = "id", required = false) final String id, @RequestBody Branch branch)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, branch.getId()))
            .body(result);
    }
    ...
    public ResponseEntity<Branch> partialUpdateBranch(
        ...
        return ResponseUtil.wrapOrNotFound(result, HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, branch.getId()));
    }
     ...
     */
    @GetMapping("/branches")
    public List<Branch> getAllBranches(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
