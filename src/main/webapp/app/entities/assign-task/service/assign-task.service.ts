...
type RestOf<T extends IAssignTask | NewAssignTask> = Omit<T, 'taskAssignmentDate' | 'taskStartDate' | 'taskDueDate'> & {
...
export class AssignTaskService {
  ...
  protected convertDateFromClient<T extends IAssignTask | NewAssignTask | PartialUpdateAssignTask>(assignTask: T): RestOf<T> {
    return {
      ...assignTask,
      taskAssignmentDate: assignTask.taskAssignmentDate?.format(DATE_FORMAT) ?? null,
      taskStartDate: assignTask.taskStartDate?.format(DATE_FORMAT) ?? null,
      taskDueDate: assignTask.taskDueDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAssignTask: RestAssignTask): IAssignTask {
    return {
      ...
      taskDueDate: restAssignTask.taskDueDate ? dayjs(restAssignTask.taskDueDate) : undefined,
    };
  }
